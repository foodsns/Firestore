...
describe("Test goods collection which is post's subcollection", () => {
    ...
    it("Can set good mark to other post2", async () => {
        const myAuth = {uid: myId, email: 'abc@example.com'}
        const postID = "post_good_ghi"
        const setQuery = getFirestore(null, true).collection("posts").doc(postID)
        await setQuery.set(
            {
                id: 'ae3f053e-e0d4-486b-af6e-3d6138d426f9',
    ...
    it("Get my goods list", async () => {
        const myAuth = {uid: myId, email: 'abc@example.com'}
        const queryResult = await Promise.allSettled((await getFirestore(myAuth).collectionGroup('goods').where("authorId", "==", `${myAuth.uid}`).get()).docs.map(async (item) => await ((await item.ref.parent.parent.get()).data())))
        await firebase.assertSucceeds(queryResult.length == 2)
        await firebase.assertSucceeds(queryResult.filter(item => item.status.fulfilled).length == 1)
    })
})